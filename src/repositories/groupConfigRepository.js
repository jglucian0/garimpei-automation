const pool = require('../infra/db');

class GroupConfigRepository {

  async registerGroup(sessionId, groupId, groupName, role, niche) {
    const query = `
      INSERT INTO group_configs (session_id, group_id, group_name, role, niche)
      VALUES ($1, $2, $3, $4, $5)
      ON CONFLICT (session_id, group_id)
      DO UPDATE SET 
        group_name = EXCLUDED.group_name, 
        role = EXCLUDED.role, 
        niche = EXCLUDED.niche;
    `;
    await pool.query(query, [sessionId, groupId, groupName, role, niche]);
    return true;
  }

  async isCollectorGroup(sessionId, groupId) {
    const query = `
      SELECT 1 FROM group_configs 
      WHERE session_id = $1 AND group_id = $2 AND role = 'coletor'
    `;
    const result = await pool.query(query, [sessionId, groupId]);
    return result.rowCount > 0;
  }
}

module.exports = new GroupConfigRepository();